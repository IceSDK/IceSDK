...
{
	ICESDK_PROFILE_FUNCTION();
	return Texture2D::Load(FileSystem::GetFileName(pPath), FileSystem::ReadBinaryFile(pPath));
}
...
Memory::Ptr<Texture2D> Texture2D::LoadRaw(const std::string &pName, size_t pWidth, size_t pHeight, bgfx::TextureFormat::Enum pTexFormat, const std::vector<uint8_t> &pData)
{
	ICESDK_PROFILE_FUNCTION();
...
Memory::Ptr<Texture2D> Texture2D::Create(const std::string &pName, const size_t pWidth, const size_t pHeight, bgfx::TextureFormat::Enum pTexFormat)
{
	ICESDK_PROFILE_FUNCTION();

	auto tex2D = std::make_shared<Texture2D>();

	tex2D->_inner = bgfx::createTexture2D(pWidth, pHeight,
										  false, 1,
										  pTexFormat,
										  pTexFormat);
