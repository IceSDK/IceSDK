...
Entity & Entity::Position(glm::vec3 pPosition) {
    ...
    auto& transformComponent = this->GetComponent<Components::TransformComponent>();
...
Entity& Entity::DefaultTransformation()
{
    ...
    auto& transformComponent = this->AddComponent<Components::TransformComponent>();
...
Entity& Entity::Sprite(Memory::Ptr<Graphics::Texture2D> pTexture, const glm::vec2& pSize)
{
    ICESDK_PROFILE_FUNCTION();
    ...
    this->AddComponent<Graphics::Components::SpriteComponent>(texSize, pTexture);

    return *this;
}

Entity& Entity::Text(const std::string& pText, size_t pFontSize, Graphics::FontFaceHandle pFontFace)
{
    ICESDK_PROFILE_FUNCTION();
    ...
    this->AddComponent<Graphics::Components::TextComponent>(pText, (uint64_t) 0, pFontSize, pFontFace);
